document.addEventListener('DOMContentLoaded', () => {
    ...
    let micReady = false; // Moved to a higher scope
    ...
    relightBtn.addEventListener('click', () => {
        ...
        // Reset instructions and Mic status with valid var references
        instructionText.innerText = "Wait a moment... preparing magic ✨";
        micStatus.textContent = "Microphone: Warming up... ⏳";
        ...
        // Reset micReady and set a warm-up delay
        micReady = false;
        setTimeout(() => {
            micReady = true;
            instructionText.innerText = "Blow hard into your microphone screen! 💨🕯️";
    ...
    enableMicBtn.addEventListener('click', async () => {
        try {
            ...
            let blowCount = 0; // Track consecutive loud inputs
            ...
            scriptProcessor.onaudioprocess = function () {
                ...
                // Increased threshold from 25 to 40 to prevent auto-blowout
                // Added sustained check to ensure it's not a random noise spike
                if (average > 40) {
                    blowCount++;
                    if (blowCount > 5) { // Require ~5 consecutive frames (approx 200ms)
                        blowOutCandles();
                        blowCount = 0;
                    }
                } else {
                    blowCount = 0; // Reset if noise drops
                }
            };
    ...
    function blowOutCandles() {
        ...
        // Show Relight Button
        console.log("Candles blown out! Scheduling Relight button...");
        setTimeout(() => {
            console.log("Showing Relight button now.");
            relightBtn.style.display = 'block';
        }, 2000); // Wait 2s before showing button
